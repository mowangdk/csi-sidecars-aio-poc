...
class IsolatedSnapshotTests(unittest.TestCase):
    def test_build_only_package_scope(self):
        script = Path(__file__).with_name("sync.sh").read_text()
        for excluded in ("lifecycle-adapter", "pkg/runtime", "pkg/csistartup", "pkg/leaderelection", "runtime_test.go"):
            self.assertNotIn(excluded, script)
        self.assertEqual(isolated_sync.MAINTAINED_PACKAGES.split(), [
    ...
    def test_removed_runtime_baseline_is_rejected_before_clone(self):
