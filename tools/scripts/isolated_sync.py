...
def main():
    ...
    work = ROOT / ".work"
    if args.update_dependencies and (args.dependency_lock or args.tooling_only or
                                    not re.fullmatch(r"1\.[0-9]+\.[0-9]+", args.update_dependencies)):
        parser.error("--update-dependencies requires 1.MINOR.PATCH and cannot replay a bundle or run tooling-only")
    ...
    if args.update_dependencies:
        previous = assembly_lock.safe_path(checkout, assembly_lock.LOCK)
        if previous.exists():
            previous.rename(run / "previous-dependencies.lock.json")
    candidate = None
    original = b""
    ...
    if result.returncode:
        ...
        print("Source, Kubernetes-family, and canonical dependency checks passed; "
