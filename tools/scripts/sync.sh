...
# The lifecycle adapter uses only the standard library and runs before assembly
# creates the root module/workspace. Unknown upstream lifecycle shapes fail sync.
GO111MODULE=off go test -count=1 -timeout=30s ./tools/cmd/lifecycle-adapter
GO111MODULE=off go build -o tmp/lifecycle-adapter ./tools/cmd/lifecycle-adapter
...
commit.message = new_message.encode()
  ...
  # Apply the maintained attacher entrypoint before deriving its AIO variant,
  # so clean and repeated assembly use the same lifecycle and flag mappings.
  ...
  for FILE in $(find pkg/${SIDECAR}/cmd/csi-${SIDECAR}/ -maxdepth 1 -name '*.go' ! -name '*_test.go' | sort); do
    ...
    # Context and lifecycle ownership are adapted by the checked Go tool below.
    ...
    tmp/lifecycle-adapter "${SIDECAR}" "${NEW_FILE}"
    add_generation_marker "${NEW_FILE}" "external-${SIDECAR}"
  done
done

# Generate AIO-only worker methods; standalone Run implementations are untouched.
tmp/lifecycle-adapter --workers .
...
symlink_from_root_to_tools tools/cmd/csi-sidecars/runtime_test.go
symlink_from_root_to_tools tools/cmd/csi-sidecars/snapshot_runtime_test.go
for FILE in tools/pkg/runtime/*.go tools/pkg/leaderelection/*.go tools/pkg/csistartup/*.go tools/pkg/resizer/pkg/csi/aio*.go tools/pkg/provisioner/pkg/owner/aio*.go; do
  symlink_from_root_to_tools "${FILE}"
done
...
# AIO-only siblings leave the vendored standalone lifecycle unchanged.
tmp/lifecycle-adapter --dependencies .
if [[ -n ${UPDATE_KUBERNETES} ]]; then
...
go test -timeout=5m ./cmd/csi-sidecars/... ./pkg/attacher/cmd/csi-attacher/config/... ./pkg/runtime/... ./pkg/leaderelection/... ./pkg/csistartup/... ./pkg/resizer/pkg/csi/... ./pkg/provisioner/pkg/owner/...
